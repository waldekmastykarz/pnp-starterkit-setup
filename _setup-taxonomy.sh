...
echo "Configuring term sets..."
  for termSet in "${termSets[@]}"; do
    ...
    if $(isError "$result"); then
      ...
      warning "    Term set already exists"
    fi
  done
success "DONE"
### /term sets
...
echo "Configuring PnP-CollabFooter-SharedLinks terms..."
...
echo "Configuring PnP-CollabFooter-SharedLinks Tools terms..."
  for term in "${terms[@]}"; do
    termId=$(getPropertyValue "$term" "ID")
    termName=$(getPropertyValue "$term" "Name")
    linkUrl=$(getPropertyValue "$term" "LinkUrl")
...
echo "Configuring PnP-Organizations terms..."
  for term in "${terms[@]}"; do
    termId=$(getPropertyValue "$term" "ID")
    termName=$(getPropertyValue "$term" "Name")
    echo "  $termName..."
